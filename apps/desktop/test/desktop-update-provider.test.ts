...
describe("desktop update feed url", () => {
  it("gives each platform its own feed file inside one release tag", () => {
    // macOS and Linux assets share the marketing-harness-latest release, so a single
    // feed name would make the last publish overwrite the other platform.
    expect(createDesktopUpdateFeedUrl("macos")).toBe(
      "https://github.com/PlaybookMediaLLC/bb/releases/download/marketing-harness-latest/desktop-version.json",
    );
    expect(createDesktopUpdateFeedUrl("linux")).toBe(
      "https://github.com/PlaybookMediaLLC/bb/releases/download/marketing-harness-latest/desktop-version-linux.json",
    );
  });
});
